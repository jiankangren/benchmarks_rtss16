...
#elif defined(__OPENMP__)
#pragma omp task //shared(W) //shared(n, a, ab)
	  compute_w_coefficients(n, a, ab, W);
#pragma omp task //shared(W) //shared(n, ab, b)
...
#elif defined(__OPENMP__)
#pragma omp task //shared(in, out) //shared(a, ab, r, m)
		 unshuffle(a, ab, in, out, r, m);
#pragma omp task //shared(in, out)  //shared(ab, b, r, m)
				 ...
				 int nW, int nWdn, int r, int m)
{
     if (i == i1 - 1) {
		 ...
		 //		 r, m, nW, nWdn * i, nWdn * m);
#elif defined(__OPENMP__)
#pragma omp task //shared(in, out, W) //shared(i, r, m, nW, nWdn)
		 ...
		 //nWdn, r, m);
#elif defined(__OPENMP__)
#pragma omp task //shared(in, out, W) //shared(i, i2, nW, nWdn, r, m)
		 fft_twiddle_gen(i, i2, in, out, W, nW,
						 nWdn, r, m);
#pragma omp task //shared(in, out, W) //shared(i2, i1, nW, nWdn, r, m)
...
#elif defined(__OPENMP__)
#pragma omp task //shared(in, out, W) //shared(a, ab, nW, nWdn, m)
		 fft_twiddle_2(a, ab, in, out, W, nW, nWdn, m);
#pragma omp task //shared(in, out, W) // shared(ab, b, nW, nWdn, m)
...
#elif defined(__OPENMP__)
#pragma omp task //shared(in, out)  //shared(a, ab, m)
		 fft_unshuffle_2(a, ab, in, out, m);
#pragma omp task //shared(in, out) //shared(ab, b, m)
			 ...
			 COMPLEX * W, int nW)
{
     ...
     if (r < n) {
		 ...
		 for (k = 0; k < n; k += m) {
			 ...
			 //     W, nW);
#elif defined(__OPENMP__)
#pragma omp task //shared(in, out, factors) //shared(m, k, nW)
...
#elif defined(__OPENMP__)
#pragma omp task shared(in, out) //shared(n, j)
