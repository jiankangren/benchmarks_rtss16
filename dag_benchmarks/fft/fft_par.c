			   ...
			   COMPLEX * in, COMPLEX * out, int r, int m)
{
     ...
     if (b - a < 16) {
	  ...
	  for (i = a; i < b; ++i) {
		 ...
		 cilk_spawn unshuffle(ab, b, in, out, r, m);
		 //	  spawn unshuffle(a, ab, in, out, r, m);
		 //	  spawn unshuffle(ab, b, in, out, r, m);
#elif defined(__OPENMP__)
#pragma omp task shared(a, ab, in, out, r, m)
		 unshuffle(a, ab, in, out, r, m);
#pragma omp task shared(ab, b, in, out, r, m)
				 ...
				 int nW, int nWdn, int r, int m)
{
     if (i == i1 - 1) {
#if defined(__CILKPLUS__)
		 cilk_spawn fft_twiddle_gen1(in + i, out + i, W,
								r, m, nW, nWdn * i, nWdn * m);
		 //spawn fft_twiddle_gen1(in + i, out + i, W,
		 //		 r, m, nW, nWdn * i, nWdn * m);
#elif defined(__OPENMP__)
#pragma omp task shared(in, out, i, W, r, m, nW, nWdn)
		 ...
		 int i2 = (i + i1) / 2;
#if defined(__CILKPLUS__)
		 cilk_spawn fft_twiddle_gen(i, i2, in, out, W, nW,
							   nWdn, r, m);
		 cilk_spawn fft_twiddle_gen(i2, i1, in, out, W, nW,
							   nWdn, r, m);
		 ...
		 //nWdn, r, m);
#elif defined(__OPENMP__)
#pragma omp task shared(i, i2, in, out, W, nW, nWdn, r, m)
		 fft_twiddle_gen(i, i2, in, out, W, nW,
						 nWdn, r, m);
#pragma omp task shared(i2, i1, in, out, W, nW, nWdn, r, m)
			       ...
			       COMPLEX * W, int nW, int nWdn, int m)
{
     ...
     if ((b - a) < 128) {
	  for (i = a, l1 = nWdn * i, kp = out + i; i < b;
		 ...
		 cilk_spawn fft_twiddle_2(ab, b, in, out, W, nW, nWdn, m);
		 //		 spawn fft_twiddle_2(a, ab, in, out, W, nW, nWdn, m);
		 //		 spawn fft_twiddle_2(ab, b, in, out, W, nW, nWdn, m);
#elif defined(__OPENMP__)
#pragma omp task shared(a, ab, in, out, W, nW, nWdn, m)
		 fft_twiddle_2(a, ab, in, out, W, nW, nWdn, m);
#pragma omp task shared(ab, b, in, out, W, nW, nWdn, m)
...
static void fft_unshuffle_2(int a, int b, COMPLEX * in, COMPLEX * out, int m)
{
     ...
     if ((b - a) < 128) {
	  ...
	  for (i = a; i < b; ++i) {
		 ...
		 cilk_spawn fft_unshuffle_2(ab, b, in, out, m);
		 //		 spawn fft_unshuffle_2(a, ab, in, out, m);
		 //		 spawn fft_unshuffle_2(ab, b, in, out, m);
#elif defined(__OPENMP__)
#pragma omp task shared(a, ab, in, out, m)
		 fft_unshuffle_2(a, ab, in, out, m);
#pragma omp task shared(ab, b, in, out, m)
			       ...
			       COMPLEX * W, int nW, int nWdn, int m)
{
     ...
     if ((b - a) < 128) {
	  for (i = a, l1 = nWdn * i, kp = out + i; i < b;
		 ...
		 cilk_spawn fft_twiddle_4(ab, b, in, out, W, nW, nWdn, m);
...
static void fft_unshuffle_4(int a, int b, COMPLEX * in, COMPLEX * out, int m)
{
     ...
     if ((b - a) < 128) {
	  ...
	  for (i = a; i < b; ++i) {
		 ...
		 cilk_spawn fft_unshuffle_4(ab, b, in, out, m);
			       ...
			       COMPLEX * W, int nW, int nWdn, int m)
{
     ...
     if ((b - a) < 128) {
	  for (i = a, l1 = nWdn * i, kp = out + i; i < b;
		 ...
		 cilk_spawn fft_twiddle_8(ab, b, in, out, W, nW, nWdn, m);
...
static void fft_unshuffle_8(int a, int b, COMPLEX * in, COMPLEX * out, int m)
{
     ...
     if ((b - a) < 128) {
	  ...
	  for (i = a; i < b; ++i) {
		 ...
		 cilk_spawn fft_unshuffle_8(ab, b, in, out, m);
				...
				COMPLEX * W, int nW, int nWdn, int m)
{
     ...
     if ((b - a) < 128) {
	  for (i = a, l1 = nWdn * i, kp = out + i; i < b;
		 ...
		 cilk_spawn fft_twiddle_16(ab, b, in, out, W, nW, nWdn, m);
...
static void fft_unshuffle_16(int a, int b, COMPLEX * in, COMPLEX * out, int m)
{
     ...
     if ((b - a) < 128) {
	  ...
	  for (i = a; i < b; ++i) {
		 ...
		 cilk_spawn fft_unshuffle_16(ab, b, in, out, m);
				...
				COMPLEX * W, int nW, int nWdn, int m)
{
     ...
     if ((b - a) < 128) {
	  for (i = a, l1 = nWdn * i, kp = out + i; i < b;
		 ...
		 cilk_spawn fft_twiddle_32(ab, b, in, out, W, nW, nWdn, m);
...
static void fft_unshuffle_32(int a, int b, COMPLEX * in, COMPLEX * out, int m)
{
     ...
     if ((b - a) < 128) {
	  ...
	  for (i = a; i < b; ++i) {
		 ...
		 cilk_spawn fft_unshuffle_32(ab, b, in, out, m);
			 ...
			 COMPLEX * W, int nW)
{
     ...
     if (r < n) {
		 ...
		 for (k = 0; k < n; k += m) {
#if defined(__CILKPLUS__)
			 cilk_spawn fft_aux(m, out + k, in + k, factors + 1,
								W, nW);
			 //spawn fft_aux(m, out + k, in + k, factors + 1,
			 //     W, nW);
#elif defined(__OPENMP__)
#pragma omp task shared(m, out, k, in, factors, W, nW)
...
#elif defined(__OPENMP__)
#pragma omp task shared(n, j, in, out)
...
struct ExeTimeData *data = NULL;
// The process that runs the last iteration will write the data 
// to a file and destroy the shared memory
int done = 0;
...
int init(int argc, char *argv[]) {
	...
	get_options(argc-1, argv+1, specifiers, opt_types, &size, &correctness, &benchmark, &help);
	...
	// Get and map the shared memory for data
    data = get_shm_mem();
	if (data == NULL) {
		return -1;
	}	
...
#pragma omp single 
		...
		// Write execution time to the shared mem
		pthread_mutex_lock(&data->mutex);
		data->data[data->count] = diff.tv_sec * USEC_IN_SEC + diff.tv_nsec/1000;
		data->count++;
		
		// Check if I am the last process
		if (data->count == TOTAL_NUM_ITERS) {
			done = 1;
		}
...
int finalize(int argc, char *argv[]) {
	...
	if (done == 0) {
		// I am not the last one, just unmap and return
		unmap_shm_mem(data);
		return 0;
	}
	// I am the last one, I must write the data to a file
	// and destroy the memory
	std::string data_folder = "data2/";
	std::stringstream ss;
	
#if defined(__CILKPLUS__)
	ss << data_folder << "cilk_" << argv[1] << "cores.dat";
#elif defined(__OPENMP__)
	ss << data_folder << "omp_" << argv[1] << "cores.dat";
#endif
	
	std::ofstream ofs(ss.str().c_str());
	
	pthread_mutex_lock(&data->mutex);
	assert(data->count == TOTAL_NUM_ITERS);
	
	for (int i=0; i<TOTAL_NUM_ITERS; i++) {
		ofs << data->data[i] << std::endl;
	}
	pthread_mutex_unlock(&data->mutex);
