...
void 
allcgrid(double **new_val, double **old, int lb, int ub)
{
  int j;
  double **rne, **rol;

  for (j=lb, rol=old+lb, rne=new_val+lb; j < ub; j++, rol++, rne++) {
...
void 
compstripe(register double **new_val, register double **old, int lb, int ub)
{
  ...
  for (a=llb; a < lub; a++) {
    for (b=1; b < ny-1; b++) {
      new_val[a][b] =   dtdxsq * (old[a+1][b] - 2 * old[a][b] + old[a-1][b])
  ...
  for (a=llb, b=ny-1; a < lub; a++)
    new_val[a][b] = randb(xu + a * dx, t);
  
  for (a=llb, b=0; a < lub; a++)
    new_val[a][b] = randa(xu + a * dx, t);
  
  if (lb == 0) {
    for (a=0, b=0; b < ny; b++)
      new_val[a][b] = randc(yu + b * dy, t);
  }
  if (ub == nx) {
    for (a=nx-1, b=0; b < ny; b++)
      new_val[a][b] = randd(yu + b * dy, t);
  }
}
...
int divide(int lb, int ub, double **new_val, double **old, int mode, int timestep)
{
  int l, r;

  if (ub - lb > leafmaxcol) {
#if defined(__CILKPLUS__)
    l = cilk_spawn divide(lb, (ub + lb) / 2, new_val, old, mode, timestep);
    r = divide((ub + lb) / 2, ub, new_val, old, mode, timestep);
    cilk_sync;
#elif defined(__OPENMP__)
#pragma omp task shared(l)
	l = divide(lb, (ub + lb) / 2, new_val, old, mode, timestep);
...
#pragma omp taskwait
    ...
    case COMP:
      if (timestep % 2)
	compstripe(new_val, old, lb, ub);
      else
	compstripe(old, new_val, lb, ub);
      return 1;
      
    case ALLC:
      allcgrid(new_val, old, lb, ub);
      return 1;
...
int heat(void)
{
  ...
  new_val = (double **) malloc(nx * sizeof(double *));
  l = divide(0, nx, new_val, old, ALLC, 0);
  ...
  /* Initialization */
  l = divide(0, nx, new_val, old, INIT, 0);
  ...
  for (c = 1; c <= nt; c++) {
	...
	l = divide(0, nx, new_val, old, COMP, c);
...
int init(int argc, char *argv[]) {
	...
	get_options(argc-1, argv+1, specifiers, opt_types, &leafmaxcol, &nx, &ny, &nt, &xu, &xo, &yu, &yo, &tu, &to, filename, &benchmark, &help);
