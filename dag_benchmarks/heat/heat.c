...
// Move allocation and initialization to this function
void heat_init(void) 
{
	/* Memory Allocation */
...
int init(int argc, char *argv[]) {
	...
	// Call allocation and initialization here
	// The heat() now only does real computation
	heat_init();
...
int run(int argc, char *argv[]) {
	...
	std::string data_folder = "/export/shakespeare/home/sonndinh/codes/fss_rtss16_code/dag_benchmarks/heat/data2/";
	std::stringstream ss;
	
#if defined(__CILKPLUS__)
	ss << data_folder << "cilk_" << "nx=" << nx << "_ny=" << ny << "_nt=" << nt << "_" << argv[1] << "cores.dat";
#elif defined(__OPENMP__)
	ss << data_folder << "omp_" << "nx=" << nx << "_ny=" << ny << "_nt=" << nt << "_" << argv[1] << "cores.dat";
#endif
...
#pragma omp single
		...
		//		std::cout << (diff.tv_sec*USEC_IN_SEC + diff.tv_nsec/NSEC_IN_USEC) << std::endl;
	}
