...
void compile_program(INSTR * pc)
{
	...
	int *A = (int*) alloca(nsquare * (2 * n + 2) * sizeof(int));
	int *b = (int*) alloca((2 * n + 2) * sizeof(int));
     int neq;
...
{
	int *nknown = (int*) alloca(c * sizeof(int));

     /* # of unknowns in this equation */
	int *unknowns = (int*) alloca(r * sizeof(int));
	int *unknowns_modulo = (int*) alloca(NMODULOS * r * sizeof(int));
...
{
     int nvar = c;
     int *known = (int*) alloca(c * sizeof(int));
     int i, j, current_var;
     int last_guessed = -1;
     /*     long score; */
     int first_guess = 1;
 ...
 */
long execute(int *grid, int *used, INSTR * pc, int m)
{
 ...
 again:
	...
	case I_GUESS:
		...
		memset(tmp, 0, nsquare*sizeof(long));

		for (i = 0; i < nsquare; ++i) {
			if (!used[i]) {
				//			ngrid = Cilk_alloca(nsquare * sizeof(int));
				//			nused = Cilk_alloca(nsquare * sizeof(int));
				ngrid = (int*) malloc(nsquare * sizeof(int));
				nused = (int*) malloc(nsquare * sizeof(int));
				memcpy(ngrid, grid, nsquare * sizeof(int));
				memcpy(nused, used, nsquare * sizeof(int));
				nused[i] = 1;
				ngrid[q] = i + 1;
				tmp[i] = cilk_spawn execute(ngrid, nused, pc, m);
				// Delay the sum of count until after cilk_sync
				//				count += tmp;
			}
		}
		cilk_sync;

		// Now, update the count value
		for (k=0; k<nsquare; k++) {
			count += tmp[k];
		}
		break;
	...
	case I_FIRST_GUESS:
		...
		for (i = 0; i < nsquare / 2; ++i) {
			if (!used[i]) {
				...
				tmp[i] = cilk_spawn execute(ngrid, nused, pc,
												 (i + i == nsquare - 1 ?
												  1 : 2));
				//				count += tmp;
			}
	...
	case I_GUESS_MOD:
		...
		// Reset the content of temporary array
		memset(tmp, 0, nsquare * sizeof(long));
	...
	case I_PRUNE_ROW:
	 ...
	 default:
		 ...
		 fprintf(stderr, "BUG: unrecognized instruction\n");
     }
...
int main(int argc, char *argv[])
{
     ...
     /* Timing. "Start" timers */
	 // ? Why they put "sync" here without any "spawned" function
	 //     cilk_sync;
     ...
     count = execute(grid, used, program, 1);
	 //     cilk_sync;
